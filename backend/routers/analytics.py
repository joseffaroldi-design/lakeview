...
# ---------------------------------------------------------------- Flyer share tracking
# Item 2 (Feb 2026): count Share Flyer clicks per menu item so the owner
# can see which dishes actually get shared. One document per menu item;
# increments an atomic counter + records the most recent share timestamp.

class _FlyerShareTracker:
    """Namespace holder for the two endpoints below — avoids helper explosion."""
    pass


@router.post("/analytics/flyer-share")
@limiter.limit("30/minute")
async def track_flyer_share(request: Request):
    body = await request.json()
    item_key = (body.get("item_key") or "").strip()
    item_name = (body.get("item_name") or "").strip()
    theme = (body.get("theme") or "").strip()
    platform = (body.get("platform") or "").strip()  # 'webshare' | 'facebook' | 'instagram'
    if not item_key:
        # No item context — accept but do not persist. Prevents error toasts
        # in the browser when the owner shares a flyer with no menu_item bound.
        return {"ok": True, "tracked": False}

    now_iso = datetime.now(timezone.utc).isoformat()
    await db.flyer_shares.update_one(
        {"item_key": item_key},
        {
            "$setOnInsert": {"item_key": item_key, "first_shared_at": now_iso},
            "$set": {"item_name": item_name or item_key, "last_shared_at": now_iso, "last_theme": theme, "last_platform": platform},
            "$inc": {"share_count": 1, f"platform_counts.{platform or 'unknown'}": 1},
        },
        upsert=True,
    )
...
@router.get("/analytics/flyer-shares")
async def get_flyer_shares(authorization: str = Header(None), session_token: str = Cookie(None)):
    await verify_session(authorization, session_token)
    docs = await db.flyer_shares.find({}, {"_id": 0}).sort("share_count", -1).to_list(500)
    total = sum(d.get("share_count", 0) for d in docs)
    return {"items": docs, "total_shares": total, "unique_items": len(docs)}
