...
import {
  ...
  Pin, Copy as CopyIcon, Folder, Lock, Unlock,
} from "lucide-react";
...
const Review = ({ getAuthHeader, job, onStartOver, onReloadTemplates, fromRecent = false }) => {
  ...
  // Sprint 22I Phase B — Lock variant. When a variant is locked the user
  // is signalling "this is the design I want to keep." The top-level
  // "Regenerate all" + "Generate copy" buttons relabel so the user can
  // tell at a glance what each action does and that their pick is safe.
  ...
  return (
    <div className="space-y-4" data-testid="designer-step-review">
      {lockedIdx !== null ? (
        <div className="flex items-center justify-between rounded-md bg-gold/15 border-2 border-gold px-3 py-2" data-testid="designer-lock-banner">
          <div className="flex items-center gap-2 text-sm text-navy">
            <Lock className="w-4 h-4 text-gold" />
            <span className="font-semibold">Design locked.</span>
            <span className="text-muted-foreground">Use &ldquo;Edit copy&rdquo; below to tweak text — the design stays put.</span>
          </div>
          <button
            type="button"
            ...
            data-testid="designer-unlock"
          >
            <Unlock className="w-3 h-3" /> Unlock
          </button>
        </div>
      ) : null}
      <Section title={lockedIdx !== null ? "Design locked — keep tweaking copy" : "Your 3 designs are ready"} icon={Wand2} testId="designer-review">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {successes.map((v) => {
            const actualIdx = (job.variations || []).indexOf(v);
            const assetUrl = `${API}/media/file/${v.asset_id}`;
            const isLocked = lockedIdx === actualIdx;
            const isDimmed = lockedIdx !== null && !isLocked;
            return (
              <div
                key={v.asset_id}
                className={`rounded-md overflow-hidden border-2 bg-card transition-opacity ${
                  isLocked ? "border-gold ring-2 ring-gold/40" : "border-navy/10"
                } ${isDimmed ? "opacity-50" : ""}`}
                data-testid={`designer-result-${v.variant || v.theme}`}
              >
                <button
                  type="button"
                ...
                <div className="p-2.5">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {isLocked ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide bg-gold text-navy px-1.5 py-0.5 rounded" data-testid={`designer-locked-badge-${v.variant || v.theme}`}>
                  ...
                  <button
                    type="button"
                    onClick={() => setLockedIdx(isLocked ? null : actualIdx)}
                    className={`w-full inline-flex items-center justify-center gap-1 text-xs font-semibold py-2 px-2 rounded mb-1.5 transition-colors ${
                      isLocked
                        ? "bg-navy text-cream hover:bg-navy/90"
                        : "bg-gold text-navy hover:bg-gold/90"
                    }`}
                    data-testid={`designer-lock-${v.variant || v.theme}`}
                    title={isLocked ? "Unlock to compare with the others" : "Lock this as the design you want to keep"}
                  >
                    {isLocked ? <><Unlock className="w-3.5 h-3.5" /> Unlock design</> : <><Lock className="w-3.5 h-3.5" /> Lock this design</>}
                  </button>
        ...
        <div className="flex flex-wrap gap-2 mt-4 items-center" data-testid="designer-bottom-actions">
          <Button
            variant="outline"
            onClick={onStartOver}
            className={lockedIdx !== null ? "border-navy/20 text-muted-foreground" : "border-navy/20"}
            data-testid="designer-start-over"
            title={lockedIdx !== null ? "Generate 3 fresh designs — your locked design stays in the Library" : "Regenerate all 3 designs with a fresh look"}
          >
            <RefreshCw className="w-4 h-4 mr-1" />
            {lockedIdx !== null ? "Try 3 more (keeps locked design)" : "Regenerate all 3 designs"}
          </Button>
          {hasCopy ? (
            <Button
            ...
            <Button
              ...
              title={lockedIdx !== null ? "Write captions / hashtags for your locked design — design stays unchanged" : "Generate captions, hashtags, and social copy"}
            >
              {generatingCopy ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Sparkles className="w-4 h-4 mr-1" />}
              {generatingCopy
                ? "Writing copy…"
                : (lockedIdx !== null ? "Edit copy (design stays locked)" : "Generate Marketing Pack Copy")}
            </Button>
          )}
        </div>
