...
import {
  ArrowLeft, LogOut, Pencil, Users, Home, Image as ImageIcon, Megaphone,
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const switchTab = (tab, subTab) => {
    const safeTab = TABS.some((item) => item.id === tab) ? tab : "home";
    setActiveTab(safeTab);
    if (safeTab === "customers") {
  ...
  return (
    <div className="dashboard-shell min-h-screen" data-testid="dashboard-shell">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        {activeTab === "home" && <HomeTab onNavigate={switchTab} />}

        {activeTab === "menu" && (
          <Suspense fallback={<TabFallback />}>
            <section data-testid="menu-tab">
              <header className="mb-8">
                <p className="ds-eyebrow mb-1">Restaurant operations</p>
                <h2 className="ds-display text-3xl sm:text-4xl">Menu</h2>
                <p className="text-sm text-navy/60 mt-2 max-w-xl">
                  Keep dishes, prices and descriptions current. Nothing extra.
                </p>
              </header>
        ...
        {activeTab === "marketing" && (
          <Suspense fallback={<TabFallback />}>
            <section data-testid="marketing-tab">
              <header className="mb-8">
                <p className="ds-eyebrow mb-1">Public website</p>
                <h2 className="ds-display text-3xl sm:text-4xl">Marketing</h2>
                <p className="text-sm text-navy/60 mt-2 max-w-xl">
                  Update the words customers see on the Lakeview website.
                </p>
              </header>
