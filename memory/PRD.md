...
## P1 Backlog (Next)
- **AI Studio Phase 3 — Restaurant Mode plugin**: one-click generation (FB/IG/TikTok/Email/SMS/Flyer) from a Menu Editor item. Implement as a pluggable module so future verticals (Moving / Event / Retail / Service) can register the same way.
- **AI Studio Phase 4**: History/status filters (Draft/Active/Archived), keyword search across all generations, Export to TXT/CSV/Clipboard.
- **AI Studio Phase 5**: Usage analytics cards (Ads Generated by month, Most Used Platform, Cost-per-generation).
